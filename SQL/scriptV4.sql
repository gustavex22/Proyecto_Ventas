...
(
ID_IMAGENES INT PRIMARY KEY NOT NULL IDENTITY(1,1),
NOMBRE NVARCHAR(100)NOT NULL,
TIPO NVARCHAR(50) NOT NULL CHECK(TIPO IN('IMAGENES_PRODUCTOS','IMAGENES_SISTEMAS')),
IMAGEN VARBINARY(MAX)NOT NULL,
FECHADESUBIDA DATETIME DEFAULT GETDATE()NOT NULL
)
...
ID_IMAGENES INT PRIMARY KEY NOT NULL IDENTITY(1,1),
NOMBRE NVARCHAR(100)NOT NULL,
TIPO NVARCHAR(50)NOT NULL CHECK(TIPO IN('CLIENTES','ADMINISTRADOR_BODEGA','TRABAJADOR_BODEGA')),
...
TIPO_CATEGORIA VARCHAR(100) CHECK (TIPO_CATEGORIA IN('ALIMENTOS-BASICOS','BEBIDAS','LIMPIEZA-ASEOPERSONAL','DULCES-SNACKS','PRODUCTOS-FRESCOS','OTROS')),
)
...
CONSTRAINT FK_PRODUCTOS_CATEGORIAS FOREIGN KEY (ID_CATEGORIA) REFERENCES CATEGORIAS_PRODUCTOS(ID_CATEGORIA)
)
...
CONSTRAINT FK_DETALLEPEDIDOS_PRODUCTOS FOREIGN KEY (ID_PRODUCTOS) REFERENCES PRODUCTOS(ID_PRODUCTO),
CONSTRAINT FK_DETALLEPEDIDOS_PEDIDOS FOREIGN KEY (ID_PEDIDO) REFERENCES PEDIDOS(ID_PEDIDO),
CONSTRAINT FK_DETALLEPEDIDOS_BODEGA FOREIGN KEY (ID_BODEGA) REFERENCES SUCURSALES_BODEGAS(ID_BODEGA)
)
...
CREATE LOGIN ADMINDATABASE 
	WITH PASSWORD = '123456789',
	DEFAULT_DATABASE = SISTEMA_CAHSANDCOLLECT,
    CHECK_POLICY = OFF,
    CHECK_EXPIRATION = OFF
	
-- Crear el usuario en la base de datos asociado al login
...
--se activa la opcion Ad Hoc Distributed Queries
